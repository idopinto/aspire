...
def main():
    ...
    if cl_args.subcommand == 'rank_pool':
        ...
        if cl_args.dataset in {'relish', 'treccovid'}:
            if cl_args.rep_type in {'sbalisentbienc'} and not cl_args.caching_scorer and cl_args.dataset in {'relish'}:
            ...
            elif cl_args.rep_type in {'sbalisentbienc'} and not cl_args.caching_scorer and cl_args.dataset == 'treccovid':
                rank_pool_sent_treccovid(root_path=cl_args.root_path, sent_rep_type=cl_args.rep_type,
                                         data_to_read=data_to_read, dataset=cl_args.dataset,
                                         run_name=cl_args.run_name)
            elif cl_args.rep_type in {'sbalisentbienc'} and cl_args.caching_scorer:
