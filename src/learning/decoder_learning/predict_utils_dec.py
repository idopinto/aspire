...
def batched_loss_ddp(model, batcher,batcher_config, loss_helper, logger, ex_fnames,use_bfloat16:bool=False):
    """
    ...
    """
    loss_batcher = batcher(config=batcher_config, ex_fnames=ex_fnames)
    with torch.no_grad():
        loss = torch.FloatTensor([0])
    if torch.cuda.is_available():
        loss = loss.cuda()
    iteration = 0
    logger.info('Dev pass; Num batches: {:d}'.format(loss_batcher.num_batches))
    with torch.inference_mode():
        for batch_ids, batch_dict in loss_batcher.next_batch():
            ...
            with torch.amp.autocast('cuda:0', dtype=dtype):
                ...
                if iteration % 100 == 0:
                    logger.info('\tDev pass; Iteration: {:d}/{:d}'.format(iteration, loss_batcher.num_batches))
...
def batched_loss(model, batcher, batcher_config,loss_helper,ex_fnames, use_bfloat16:bool=False):
    """
    Make predictions batch by batch.
    :param model: the model object with a predict method.
    :param batcher: reference to model_utils.Batcher class.
    ...
    """
    # Intialize batcher.
    loss_batcher = batcher(batcher_config, ex_fnames=ex_fnames)
    with torch.no_grad():
        loss = torch.FloatTensor([0])
    if torch.cuda.is_available():
        loss = loss.cuda()
